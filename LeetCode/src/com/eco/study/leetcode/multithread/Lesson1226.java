...
//        int left = getLeft(4,0,philosopher);
        int left = (philosopher + 1) % 5;
        boolean end = true;
        while(end){

            lock.lock();
            if(f[left].tryAcquire() && f[philosopher].tryAcquire()){
                pickLeftFork.run();
                pickRightFork.run();
                eat.run();
                putLeftFork.run();
    ...
    //向左转 获取叉子，   向左转 越来越大 ，转到最大就变小
    private int getLeft(int max,int min,int t){
        if(t+1 >max){
            return min;
        }
        return  t+1;
